...
def generate_material_object(material):
    # Get name to select material type
    ...
    if implementation:
        ...
        elif ('lambert' in name) or ('lam:' in name):
...
def generate_texture_object(texture):
    ...
    if type(texture) is FbxFileTexture:
        src_path = texture.GetFileName()
    else:
        src_path = getTextureName( texture )

    fullpath = replace_inFolder2OutFolder( src_path )

    url = os.path.relpath( fullpath , outputFolder )
    ...
    if node.GetNodeAttribute() == None:
        ...
        if attribute_type == FbxNodeAttribute.eMesh or \
           ...
           attribute_type == FbxNodeAttribute.ePatch:
            ...
            embed_name = getPrefixedName(node, 'Ebd')
...
def generate_geometry_dict_from_hierarchy(node, geometry_dict):
    if node.GetNodeAttribute() == None:
        ...
        if attribute_type == FbxNodeAttribute.eMesh:
            ...
            geometry_name = getPrefixedName( node, 'Geo' )
...
def generate_mesh_object(node):
    ...
    output = {
      'geometry': getPrefixedName( node, 'Geo' ),
...
def copy_textures(textures):
    ...
    for key in textures:
        ...
        dest_path = textures[key]['fullpath']

        src_path = replace_OutFolder2inFolder(dest_path)

        print('[+] copy texture : '+textures[key]['url'])
        print('    from : '+src_path)
        print('    to   : '+dest_path)

        # if texture has been copied : continute
        if src_path in texture_dict:
            continue

        # if nothing is found on src_path
        if not os.path.exists(src_path):
            print("copy_texture error: we can't find this texture at " + src_path)
            continue
        ...
        try:
            ...
            # take a note for this source_path
            texture_dict[src_path] = True
        except IOError as e:
            print ("I/O error({0}): {1} {2}").format(e.errno, e.strerror, dest_path)
...
def cleanup_list(_list):
    def f(x):
        if isinstance(x, numbers.Number):
            return cleanup_number(x)
        elif isinstance(x, list):
            return cleanup_list(x)
        else:
            f = float(x)
            if math.isnan(f):
                return x
            else:
                return cleanup_number(f)
    return map(f,_list)
    # # d[k] = map(f,v)
    # print map(f,v)
def cleanup(_dict):
